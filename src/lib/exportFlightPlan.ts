...
interface ExportFlightPlanData {
  ...
  waypoints: Waypoint[];
  notes: string;
  signature?: { imageDataUrl: string; name: string; title: string };
}
...
<body>
  ...
  ${data.signature ? `
  <div style="margin-top:20px;display:flex;align-items:flex-end;gap:30px">
    <div style="flex:1">
      <img src="${data.signature.imageDataUrl}" style="max-height:60px;margin-bottom:4px" />
      <div style="border-top:1px solid #333;padding-top:3px;font-size:7pt;color:#555;text-transform:uppercase">
        ${data.signature.name}${data.signature.title ? ' — ' + data.signature.title : ''}
      </div>
    </div>
    <div style="flex:0 0 140px;border-top:1px solid #333;padding-top:3px;font-size:7pt;color:#555;text-transform:uppercase">Date: ${date}</div>
  </div>` : `
  <div class="sig-line">
