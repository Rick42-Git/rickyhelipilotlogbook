...
export function exportDutyCalcPDF(data: DutySummary) {
  const rows = data.rows.map(r => `
    <tr style="${r.exceeded ? 'background:#fef2f2;' : ''}">
      <td>${r.date}</td>
      ...
      <td style="text-align:center;font-weight:700;color:${r.exceeded ? '#dc2626' : '#16a34a'}">${r.exceeded ? '✗' : '✓'}</td>
    </tr>`).join('');
...
<div class="footer">
  ...
  <span class="${data.exceedCount > 0 ? 'danger' : 'success'}" style="font-weight:700">${data.exceedCount > 0 ? `⚠ ${data.exceedCount} FDP LIMIT(S) EXCEEDED` : '✓ ALL WITHIN LEGAL LIMITS'}</span>
</div>
</body></html>`;
